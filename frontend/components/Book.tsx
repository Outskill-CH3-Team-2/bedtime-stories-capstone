...
const Book: React.FC<BookProps> = ({
  ...
  // Measure the outer wrapper → feed half-width to PageFlip
  ...
  // Build transparent blank pages just so PageFlip has something to flip
  ...
  // Initialise / re-initialise PageFlip when dimensions change
  useEffect(() => {
  ...
  // When scene changes → add 2 blank pages + flip
  useEffect(() => {
    ...
    if (scene.narration_audio_b64 && audioRef.current) {
      audioRef.current.src = `data:audio/mp3;base64,${scene.narration_audio_b64}`;
  ...
  return (
    <div style={{
      ...
      {/* ── StPageFlip canvas (blank pages, flip animation only) ── */}
      <div
        ref={wrapperRef}
        style={{
          position: 'absolute',
      ...
      {/* ── LEFT page content: illustration ── */}
      <div style={{
        position: 'absolute',
        top: 'calc(8% - 4%)',
        bottom: '9%',
        ...
        {/* Illustration — 30% larger by reducing padding */}
      ...
      {/* ── RIGHT page content: title + text + choices ── */}
      <div style={{
        position: 'absolute',
        top: 'calc(8% - 4%)',
        bottom: '8%',
        ...
        <h2 style={{
          ...
          margin: '0 0 4% 0',
          paddingBottom: '3%',
          borderBottom: '1px solid rgba(61,31,13,0.18)',
          lineHeight: 1.2,
          flexShrink: 0,
        }}>
          {chapterLabel}
          ...
          {/* Story text — capped height so choices always visible */}
          <div style={{
            flex: '0 1 auto',
            maxHeight: '45%',
            overflowY: 'auto',
          ...
          {/* Choices — shrinks to fit, never pushed off page */}
