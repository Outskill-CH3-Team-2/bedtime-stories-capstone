...
export const storyService = {
  ...
  async startStory(idea: string, childInfo: any): Promise<{ sessionId: string; jobId: string }> {
    const p = childInfo.personalization ?? {};
    const response = await fetch(`${API_BASE}/api/v1/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        prompt:      idea,
        child_name:  childInfo.name  || 'Friend',
        age_group:   String(childInfo.age ?? 5),
        story_length: '10_minutes',
        voice:       'onyx',
        // Personalization — map camelCase StoryConfig → snake_case backend fields
        favourite_colour:     p.favoriteColor    || '',
        favourite_animal:     p.favouriteAnimal  || '',
        favourite_food:       p.favoriteFood     || '',
        favourite_activities: p.favoriteActivity ? [p.favoriteActivity] : [],
        pet_name:             p.petName          || '',
        pet_type:             p.petType          || '',
        place_to_visit:       p.placeToVisit     || '',
      }),
    });
    if (!response.ok) {
  ...
  async getResult(jobId: string): Promise<Scene> {
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => null);
      const detail = body?.detail ?? `Failed to get result (${response.status})`;
      throw new Error(detail);
    }
