...
def sanitize_text_field(value: str | None, max_len: int, field_name: str) -> str:
    """
    Sanitize a free-text prompt field (story_idea, choice_text, prompt).
    Returns a cleaned string, or empty string if injection detected or None passed.
    """
    if not value:
        return ""
    cleaned = _clean_str(value, max_len)
...
def sanitize_input(config: ChildConfig) -> ChildConfig:
    ...
    p["place_to_visit"] = _safe_str(p.get("place_to_visit", ""), 50, "place_to_visit")
    ...
    try:
        ...
        print(f"[filters.py] ChildConfig reconstruction failed ({type(e).__name__}): {e} — using safe defaults")
        return ChildConfig(child_name="Friend", child_age=5)
