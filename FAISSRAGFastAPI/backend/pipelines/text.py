...
def build_prompt(config: ChildConfig, messages: list[dict], step_number: int, story_idea: str = "", rag_context: Optional[str] = None) -> list[dict]:
    prompts = _get_prompts()

    # Format personalization — only include non-empty fields to avoid "Loves , color , food ."
    p = config.personalization
    parts: list[str] = []
    if p.favourite_animal:
        parts.append(f"loves {p.favourite_animal}")
    if p.favourite_colour:
        parts.append(f"favourite colour is {p.favourite_colour}")
    if p.favourite_food:
        parts.append(f"loves eating {p.favourite_food}")
    details = (", ".join(parts).capitalize() + ".") if parts else ""
    if p.place_to_visit:
    ...
    print(
        f"[build_prompt] step={step_number}  idea_len={len(story_idea)}  "
...
async def generate_text(messages: list[dict]) -> str:
    ...
    try:
        ...
        print(f"[text.py] Text generation failed ({type(e).__name__}): {e}")
