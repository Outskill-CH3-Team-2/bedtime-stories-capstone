...
class GenerateRequest(BaseModel):
    ...
    story_idea: Optional[str] = Field(None, max_length=500)
    protagonist_image_b64: Optional[str] = Field(None, max_length=2_000_000)  # ~1.5 MB

    # --- next chapter only ---
    session_id: Optional[str] = Field(None, max_length=64)
    choice_text: Optional[str] = Field(None, max_length=200)   # branch to generate

    # --- history commit (next chapter only, first call of a new round) ---
    prev_job_id: Optional[str] = Field(None, max_length=64)
    prev_choice_text: Optional[str] = Field(None, max_length=200)
